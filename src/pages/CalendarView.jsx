                ...
                {payments.slice(0, 10).map((p) => (
                  <tr key={p.id}>
                    <td>
                      ...
                      <small className="text-muted">{formatIndianDate(p.dueDate)}</small>
                    </td>
                    <td>
                      <span className={`badge rounded-pill ${p.status === 'Paid' ? 'bg-success text-white' : 'bg-warning text-dark'}`}>
                        {p.status}
                      </span>
                    </td>
                    <td className="text-dark fw-semibold">{p.loanName}</td>
                    <td className="fw-bold text-success">₹{Number(p.amount).toLocaleString('en-IN')}</td>
                    <td>
                      <button
                        className="btn btn-outline-primary btn-sm rounded-3 px-2 py-1"
